...
5. **Monitor dependency updates** (as of Feb 20, 2026: both `npm audit --omit=dev --audit-level=high` and `npm audit --audit-level=high` report **0 vulnerabilities** after adding npm `overrides` for transitive `minimatch`/`glob`; verify lockfile updates on each dependency refresh)
