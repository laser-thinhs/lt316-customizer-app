...
export function enforcePolicy(document: unknown, zone: { widthMm: number; heightMm: number }, policy: PolicyMode) {
  ...
  for (const obj of objects) {
    ...
    obj.widthMm = scaledWidth;
    obj.heightMm = scaledHeight;
    obj.xMm = Math.max(0, Math.min(x, zone.widthMm - scaledWidth));
    obj.yMm = Math.max(0, Math.min(y, zone.heightMm - scaledHeight));
